...
import { getDirectorMovies } from '../request/soap';

import Tile from '../components/tile';
import Loader from '../components/loader';
import { movieIsUHD } from '../utils';

export default function movieDirectorRoute() {
  return TVDML.createPipeline()
    ...
    .pipe(
      TVDML.passthrough(({ id }) =>
        getDirectorMovies(id).then(movies => ({ movies })),
      ),
    )
    .pipe(
                ...
                <shelf centered="true">
                  <section>
                    <monogramLockup disabled="true">
                      ...
                      />
                      <title>{name}</title>
                      <subtitle>{i18n('director-title')}</subtitle>
                    </monogramLockup>
                  </section>
                </shelf>
                <grid>
                  <header>
                    <title>{i18n('director-movies')}</title>
                  </header>
                  <section>
                    {movies.map(movie => {
                      const {
