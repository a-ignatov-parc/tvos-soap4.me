...
export default function actorRoute(title) {
  return TVDML.createPipeline()
    .pipe(
      TVDML.passthrough(({ navigation: { id, actor, poster } }) => ({
        id,
        actor,
        poster,
      })),
    ...
    .pipe(
      TVDML.render(({ actor, poster }) => (
        <Loader title={actor} heroImg={poster} />
      )),
    )
    .pipe(
      TVDML.passthrough(({ id }) =>
        getActorInfo(id).then(({ bio, soap: tvshows }) => ({ bio, tvshows })),
      ),
    )
    .pipe(
      TVDML.render(({ bio, tvshows }) => {
        const { name_en: actorName, image_original: actorImage } = bio;
