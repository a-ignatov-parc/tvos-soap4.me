...
import { getSpeedTestServers, saveSpeedTestResults } from '../request/soap';
...
const flags = flagsContext.keys().reduce((result, moduleName) => {
    ...
    TVDML.render(
      TVDML.createComponent({
        ...
        componentDidMount() {
          ...
          Promise.all([getSpeedTestServers(), waitForAnimations]).then(
            ([servers]) => this.setState({ loading: false, servers }),
          );
        },
        ...
        render() {
          ...
          const serversList = Object.keys(servers).map(id => ({
            id,
            file: servers[id],
          }));
          ...
          return (
            <document>
              <stackTemplate>
                <banner>
                  <title>{i18n('speedtest-caption')}</title>
                </banner>
                <collectionList>
                  <shelf
                    centered="true"
                  ...
                  <row style="tv-align: center">
                    {running ? (
                      ...
                      <button onSelect={this.onStart}>
                        <text>{i18n('speedtest-begin')}</text>
                      </button>
                    )}
                  </row>
                  <row style="tv-align: center">
                    <text style="tv-text-style: footnote; color: rgb(84, 82, 80)">
        ...
        onStart() {
          ...
          Object.keys(this.state.servers).forEach(id => {
          ...
          chain
            .then(results => {
              ...
              const hasProperResults = Object.keys(results).some(
                id => results[id] !== '1.00',
              );
            ...
            .catch(() => {
              const promise = TVDML.renderModal(
                <document>
                  <alertTemplate>
                    <title>{i18n('speedtest-error-title')}</title>
                    <description>
