...
import { getWriterMovies } from '../request/soap';

import Tile from '../components/tile';
import Loader from '../components/loader';
import { movieIsUHD } from '../utils';

export default function movieWriterRoute() {
  return TVDML.createPipeline()
    ...
    .pipe(
      TVDML.passthrough(({ id }) =>
        getWriterMovies(id).then(movies => ({ movies })),
      ),
    )
    .pipe(
                ...
                <shelf centered="true">
                  <section>
                    <monogramLockup disabled="true">
                      ...
                      />
                      <title>{name}</title>
                      <subtitle>{i18n('writer-title')}</subtitle>
                    </monogramLockup>
                  </section>
                </shelf>
                <grid>
                  <header>
                    <title>{i18n('writer-movies')}</title>
                  </header>
                  <section>
                    {movies.map(movie => {
                      const {
